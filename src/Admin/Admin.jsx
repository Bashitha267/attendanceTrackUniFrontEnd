                ...
                <form className="space-y-4" onSubmit={handleSubmitCourse}>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <input
                      ...
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-600 focus:outline-0"
                    />
                    <input
                      type="text"
                      name="subjectCode"
                      placeholder="Course ID"
                      value={formDataCourse.subjectCode}
                      onChange={handleInputChangeCourse}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-600 focus:outline-0"
                    />
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <input
                      ...
                      value={formDataCourse.batchYear}
                      onChange={handleInputChangeCourse}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-600 focus:outline-0"
                    />
                    <select
                      name="year"
                      value={formDataCourse.year}
                      onChange={handleInputChangeCourse}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-600 focus:outline-0"
                    >
                    ...
                    <input
                      ...
                      value={formDataCourse.lecturerId}
                      onChange={handleInputChangeCourse}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-600 focus:outline-0"
                    />
                  </div>
                  <div className="grid grid-cols-2">
                    <input
                      type="text"
                      name="subpinCode"
                      placeholder="Subject Pin Code"
                      value={formDataCourse.subpinCode}
                      onChange={handleInputChangeCourse}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-600 focus:outline-0"
                    />
                  </div>
                  ...
                  <div className="flex justify-end space-x-3">
                    ...
                    <button
                      type="submit"
                      className="px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-900 min-w-[110px]"
                    >
                      {courseLoading ? (
                        <Loader2Icon
                          size={24}
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
              ...
              <div className="flex flex-wrap gap-3 w-full md:w-auto">
                <select
                  ...
                  className="w-full sm:w-auto px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500"
                >
            ...
            <div className="space-y-4">
              ...
              {!subjectsloading &&
                filteredSubjects.map((course) => (
                  <div
                    key={course._id}
                    className="bg-white rounded-lg shadow-lg hover:bg-gray-50 p-4 border-2 border-purple-200 hover:border-purple-400"
                    onClick={() =>{ setCourseID(course._id),setActiveTab("studentList")}}
                  >
                    <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                      <div>
                        ...
                        <p className="text-gray-800">
                          Lecture ID: {course.lecturerId}
                        </p>
                        <p className="text-gray-800">
                          Subject Pin Code: {course.subpinCode}
                        </p>
            ...
            <div className="relative">
              ...
              <div className="absolute left-1/2 -translate-x-1/2 -bottom-16 transform">
                <img
                  ...
                  src={
                    profileData.img ||
                    "https://res.cloudinary.com/dnfbik3if/image/upload/v1757522500/Gemini_Generated_Image_35u0bk35u0bk35u0_mbqnnr_og0ikh.png"
                  }
                  alt="Admin"
                />
              </div>
                ...
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    ...
                    <a
                      ...
                      className="text-sm font-medium text-gray-700 hover:text-purple-600 break-all"
                    >
                      {profileData.email}
                    </a>
                  </div>
                ...
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    ...
                    <p className="text-sm font-medium text-gray-700">
                      {profileData.contact_no || "Not Available"}
                    </p>
                  </div>
                </div>
                <div className="flex items-center">
          ...
          <div className="flex flex-col gap-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              ...
              <div className="relative w-full sm:w-auto">
                <button
                  onClick={() => setOpenApprovefilter(!openApproveFilter)}
                  className="text-lg inline-flex items-center justify-between w-full sm:w-auto px-4 py-1 border-2 border-purple-600 rounded-lg"
                >
                  Filter{" "}
                  <svg
                ...
                <div
                  className={`${
                    openApproveFilter
                      ? "z-10 bg-white divide-y divide-gray-100 rounded-lg shadow-sm w-44 absolute right-0 mt-2"
                      : "hidden"
                  }`}
                >
                    ...
                    {activeroles.map((role) => {
                      ...
                      return (
                        <li key={role.id}>
                          <div
                            onClick={() => {
                              setActiveApprove(role.id);
                              setOpenApprovefilter(false);
                            }}
                            className={`${
                              activeApprove === role.id
                                ? "bg-purple-700 text-white"
                                : ""
                            } px-4 py-2 flex gap-4 items-center hover:bg-purple-400 hover:text-white cursor-pointer`}
                          >
                            <RoleIcon size={20} />
            ...
            <div className="flex flex-col gap-4">
              ...
              {filteredApprovels.map((request) => (
                <div
                  key={request.reg_no}
                  className="flex flex-col md:flex-row justify-between gap-4 p-4 shadow-md rounded-lg bg-white hover:bg-gray-50"
                >
                  <div className="flex-grow space-y-2">
                    ...
                    <div className="text-sm text-gray-500">
                      Role:{" "}
                      <span className="font-medium text-purple-700 capitalize bg-purple-100 px-2 py-1 rounded-full">
                        {request.role}
                      </span>
                    </div>
                  </div>
                  <div className="flex gap-3 self-end md:self-center flex-shrink-0">
                    <button
                      className="bg-green-100 border-2 border-green-500 rounded-xl px-3 py-2 text-sm font-semibold text-green-800 hover:bg-green-200"
                      onClick={() => addApprove(request.reg_no)}
                    >
                      Approve
                    </button>
                    <button
                      className="bg-red-100 border-2 border-red-500 rounded-xl px-3 py-2 text-sm font-semibold text-red-800 hover:bg-red-200"
                      onClick={() => removeUser(request.reg_no)}
                    >
                      Remove
                    </button>
                  </div>
      ...
      {subject ? (
        <>
          ...
          <div className="  rounded-lg p-4 mb-4">
            <h2 className="text-xl font-semibold mb-1">{subject.name} ({subject.subjectCode})</h2>
            <p className="text-gray-600 text-lg">
              Lecturer: {subject.lecturerId} | Year: {subject.year} | Semester: {subject.semester} | Batch: {subject.batchYear}
            </p>
          </div>
          <div className="text-xl font-semibold mb-1 px-4 pb-2 underline-offset-2 underline">Enrolled Student List</div>
          {/* Students List */}
          {studentList.length > 0 ? (
            <div className="space-y-2 ">
              {studentList.map((studentId) => (
                <div
                  key={studentId}
                  className="flex justify-between items-center bg-purple-50 shadow p-3 py-4 rounded hover:bg-purple-200 transition max-w-2xl border-2 border-purple-50"
                >
                  <span className="px-3 font-semibold font-sans text-lg">{studentId}</span>
                  <Trash2Icon
                    className="w-5 h-5 text-red-500 cursor-pointer hover:text-red-700 mr-3"
        ...
        <p className="text-gray-500">Loading subject info...</p>
      )}
    </div>
  );
      default:
